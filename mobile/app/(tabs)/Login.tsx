...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 1. Firebase Auth හරහා Login වීම (මෙතනදී Server unreachable error එක එන්නේ නැහැ)
      const userCredential = await signInWithEmailAndPassword(auth, email, password);
      const user = userCredential.user;

      console.log('Login Auth Success:', user.uid);

      // 2. Firestore එකෙන් මේ User ගේ Role එක (Seller/Buyer) කියවා ගැනීම
      ...
      // වැරදි විස්තර දුන්නොත් එන Error එක
